...
LogicalResult vector_broadcast_rule(RewriteContext &ctx, Operation &op,
  ...
  if (auto src_ty = dyn_cast<VectorType>(broadcast_op.getSourceType())) {
    ...
    broadcast_op.erase();
    return success();
  } else if (layout_out.bitwidth() ==
                 broadcast_op.getSourceType().getIntOrFloatBitWidth() &&
             broadcast_op.getSourceType().getIntOrFloatBitWidth() < 32) {
    // Broadcasting the scalar with narrower type involves first converting it
    // to i32, followed by broadcasting i32 to the target shape. Finally,
    // bitcasting i32 vector back to the original narrower type vector.
    ...
    if (!src_ty.isSignedInteger(bitwidth)) {
